...
def simulate_trades(df: pd.DataFrame, initial_capital: float = 100_000.0) -> dict:
    ...
    # If still in position at end, mark as open/unrealised
    final_capital = (shares * df['Close'].iloc[-1]) if position_open else capital
...
def _generate_synthetic_sentiment(df: pd.DataFrame, seed: Optional[int] = None) -> pd.Series:
    ...
    random_sent = (random_sent - random_sent.min()) / (random_sent.max() - random_sent.min())
    
    # 2. Momentum-based component
    m_min, m_max = momentum.min(), momentum.max()
    m_norm = (momentum - m_min) / (m_max - m_min) if m_max > m_min else np.full(n, 0.5)
